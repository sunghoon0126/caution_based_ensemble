...
class OpenAIClient:
    def __init__(
        ...
        if openai_api_base is None:
            openai_api_base = ["XXXX"] * len(openai_key)
            ...
            if os.path.exists(save_path) and not self.trial_run:
                with codecs.open(save_path) as f:
                    try:
                        ...
                        print(save_path)
                        raise Exception(f"Error loading response from file: {save_path}")
...
def run_openai_inference(
    ...
    request_limit: Optional[int] = 1000000000000,
    request_limit_period: Optional[int] = 1000000000,
    max_error_count: Optional[int] = 100,
    trial_run=False,
    dump_individual_rsp=True,
):
...
if __name__ == "__main__":
    ...
    run_openai_inference(
        ...
        openai_api_base=os.environ.get("OPENAI_API_BASE", "XXXX"),
